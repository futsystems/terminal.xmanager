...
{
    public partial class fmHistSettlementStatistic : ComponentFactory.Krypton.Toolkit.KryptonForm, IEventBinder
    {
        public fmHistSettlementStatistic()
        {
            InitializeComponent();
        ...
        {
           return string.Format("{0}_{1}_SettlementStatistic",inputAccount.Text, string.Format("{0}-{1}", Start, End));
        }
        ...
        void  btnQry_Click(object sender, EventArgs e)
        {
            ...
            CoreService.TLClient.ReqQrySettlementStatistic(inputAccount.Text, Start, End,direct.Checked);
        ...
        public void OnInit()
        {
            CoreService.EventCore.RegisterCallback(Modules.AgentManager, Method_AGENT_MGR.QRY_STATISTIC_SETTLEMENT, this.OnQryStatisticSettlements);
        }

        public void OnDisposed()
        {
            CoreService.EventCore.UnRegisterCallback(Modules.AgentManager, Method_AGENT_MGR.QRY_STATISTIC_SETTLEMENT, this.OnQryStatisticSettlements);
        }

        void OnQryStatisticSettlements(string jsonstr, bool islast)
        {
            SettlementStatistic obj = CoreService.ParseJsonResponse<SettlementStatistic>(jsonstr);
            if (obj != null)
            {
                GotSettlement(obj);
        ...
        void GotSettlement(SettlementStatistic settle)
        {
            if (InvokeRequired)
            {
                Invoke(new Action<SettlementStatistic>(GotSettlement), new object[] { settle });
            }
            else
            {
        ...
        private void InitTable()
        {
            ...
            gt.Columns.Add(TAG, typeof(SettlementStatistic));
