...
{
    public static class AgentExtend
    {
        /// <summary>
        ...
        /// <returns></returns>
        public static decimal GetExchangeRate(this AgentSetting agent, CurrencyType srcCurrency)
        {
            //品种货币与帐户货币一直则返回1
            if (srcCurrency == agent.Currency) return 1;

            //帐户货币为主货币
            if (agent.Currency == CurrencyType.RMB)
            {
                //获得品种货币对应的汇率 返回中间汇率
            ...
            else
            {
                ...
                ExchangeRate accRate = CoreService.BasicInfoTracker.GetExchangeRate(agent.Currency);
