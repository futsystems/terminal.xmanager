...
{
    public class ChinSpell
    {
        /// <summary>
        /// 中文字符组成的字符串数组——收录了21171个汉字
        /// </summary>
        private static string[] strChinese =
        {
            #region 汉字字符串列表
            ...
            "C铲阐蒇剷嵼滻幝蕆諂閳簅冁繟醦鏟闡囅灛讇忏硟摲懴颤懺羼韂伥昌娼淐猖菖阊晿椙琩裮锠錩鲳鯧鼚兏肠长苌尝偿常徜瓺萇甞腸嘗嫦瑺膓鋿償嚐蟐鲿鏛鱨厂场昶惝敞僘厰廠氅鋹怅玚畅倡鬯"   , 
            ...
            "C瘁粹翠膵膬竁襊臎邨村皴澊竴存刌忖寸籿搓瑳遳磋撮蹉醝髊虘嵯嵳痤矬蒫鹾鹺齹脞剉剒厝夎挫莝莡措逪棤锉蓌错銼刂刹畜曾膪澶骣粢唱"   , 
        ...
        /// <summary>
        /// 获得一个字符串（包含汉字、字母等字符）的汉语拼音码——不是汉字则返回该字符
        /// </summary>
        /// <param name="strText">字符串内容（一般是汉字)</param>
        /// <returns>拼音首字母组成的字符串,该字符串只包含大写的英文字母</returns>
        public static string GetChinSpell(string strText)
        {
            #region 引用的方法一（已注释）
            ...
            //for (int i = 0; i < len; i++)
            //{
            //    sbText.Append(GetFirSpell(strText.Substring(i, 1)));
            //}
            ...
            if (strText == null || strText.Length == 0)
                return strText;
            System.Text.StringBuilder sbSpel = new System.Text.StringBuilder();
            foreach (char chText in strText)
            {
                // 若是字母则直接输出 
                if ((chText >= 'a' && chText <= 'z') || (chText >= 'A' && chText <= 'Z'))
                    sbSpel.Append(char.ToUpper(chText));
                else if ((int)chText >= 19968 && (int)chText <= 40869)
                {
                    // 若字符Unicode编码在编码范围则 查汉字列表进行转换输出 
                    foreach (string strList in strChinese)
                    {
                        if (strList.IndexOf(chText) > 0)
                        {
                            sbSpel.Append(strList[0]);
                            break;
                        }
                    }
                }
        ...
        #region 方法一——只能取到常用字拼音的首字母
        /// <summary>
        /// 取得一个汉字的拼音首字母
        /// </summary>
        /// <param name="cnChar">一个字符（一般是汉字）</param>
        /// <returns>拼音首字母</returns>
        private static string GetFirSpell(string cnChar)
        {
